...
func CreateUser(u *models.User) error {
	query := `INSERT INTO users (name, email, password_hash, role, unit, phone, can_crud, availability, avatar_url, created_at) 
              VALUES (?, ?, ?, ?, ?, ?, ?, 'Online', ?, NOW())`
	res, err := config.DB.Exec(query, u.Name, u.Email, u.PasswordHash, u.Role, u.Unit, u.Phone, u.CanCRUD, u.AvatarURL)
	if err != nil {
		return err
	}
...
func GetAllUsers() ([]models.User, error) {
	rows, err := config.DB.Query("SELECT id, name, email, role, unit, availability, COALESCE(avatar_url, '') FROM users")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func GetUsersByUnit(unit string) ([]models.User, error) {
	...
	rows, err := config.DB.Query(query, unit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func UpdateUser(id uint, u models.User) error {
	...
	_, err := config.DB.Exec(query, u.Name, u.Unit, u.Phone, u.Role, u.CanCRUD, u.AvatarURL, id)
	return err
}

func UpdateAvailability(userID uint, status string) error {
	_, err := config.DB.Exec("UPDATE users SET availability = ? WHERE id = ?", status, userID)
	return err
}
